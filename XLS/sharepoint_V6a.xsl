...
$(function() {
 ...
 <xsl:sort select="CO_custName"/>
    ...
    <tr bgcolor="{$altColor}"   >
		...
		<td>
		<!-- <xsl:value-of  select="ShippingComments_UD_SC_SHIPPIN" /> -->
		
				<xsl:choose>
                    <xsl:when test="ShippingComments_UD_SC_SHIPPIN != ''  ">
					 <!-- <xsl:value-of  select="ShippingComments_UD_SC_SHIPPIN" /> -->
					
					<form enctype="multipart/form-data" method="post" >
						<input type="hidden" id="{APP_CONTRACT_NUMBER}"     value="{ShippingComments_UD_SC_SHIPPIN}"       /> 
						<input type="button" onclick="getComments(document.getElementById('{APP_CONTRACT_NUMBER}').id)" value="Get Comment"/> 
					</form>
